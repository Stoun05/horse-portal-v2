...
function MainCard({horse}:{horse:Horse}) { return <Link href={`/atlar/${horse.id}`} title={`${horse.name} — pasportyny aç`} className="block w-full rounded-2xl border-2 border-[#07553a] bg-[#07553a] p-5 text-white shadow-xl transition hover:-translate-y-1 hover:bg-[#043d2a] hover:shadow-2xl"><div className="mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-white/20 text-3xl">🐎</div><h3 className="text-2xl font-black text-white">{horse.name}</h3><p className="font-bold text-white">{horse.breed}</p><p className="mt-3 text-sm font-semibold text-white">{horse.year} · {horse.color}</p><p className="text-sm font-semibold text-white">{horse.code}</p><span className="mt-3 block text-xs font-black text-emerald-100">Pasportyna geçmek üçin bas →</span></Link> }
